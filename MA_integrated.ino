...
// =================== I²C CONFIG ===================
#define I2C_SLAVE_ADDR 0x08  // I²C slave address
float angle = NAN;           // last computed fused angle
float distance = NAN;        // last computed fused distance
...
static void maybeEmit(uint32_t now){
  ...
  // update globals for I²C
  angle = meanAngle;
  distance = meanFused;

  // one clean line
  Serial.printf("angle = %.1f  distance = %.3f  s1 = %.3f  s2 = %.3f  m = %.3f\n",
